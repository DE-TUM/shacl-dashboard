...
def get_violations_by_shape():
    ...
    graph_uri = request.args.get("graph_uri", default="http://ex.org/ValidationReport")
    if not shape_name:
        raise ValidationError('shape_name is required')
    graph_uri = validate_uri(graph_uri, "graph_uri")
...
def get_shape_details():
    ...
    if not node_shape_names or not isinstance(node_shape_names, list):
        raise ValidationError('node_shape_names must be a non-empty list')
...
def get_constraints_count():
    ...
    nodeshape_name = request.args.get('nodeshape_name')
    if not nodeshape_name:
        raise ValidationError("Parameter 'nodeshape_name' is required")
    nodeshape_name = validate_uri(nodeshape_name, "nodeshape_name")
...
def get_violations_distribution():
    ...
    num_bins = validate_positive_integer(num_bins, "num_bins")
